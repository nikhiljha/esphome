    ...
    cv.Schema(
        {
            ...
            cv.Optional(CONF_SOFTWARE_VERSION, default=1): cv.int_range(
...
def _set_matter_sdkconfig(config):
    ...
    # OTA Requestor support — allows the device to receive firmware updates
    # over the Matter fabric from an OTA Provider (e.g. Home Assistant).
    add_idf_sdkconfig_option("CONFIG_ENABLE_OTA_REQUESTOR", True)
    add_idf_sdkconfig_option("CONFIG_OTA_AUTO_REBOOT_ON_APPLY", True)
    add_idf_sdkconfig_option("CONFIG_OTA_AUTO_REBOOT_DELAY_MS", 5000)

    # Software version — OTA provider only sends updates with a higher version.
    add_idf_sdkconfig_option(
        "CONFIG_DEVICE_SOFTWARE_VERSION_NUMBER", config[CONF_SOFTWARE_VERSION]
    )


def _write_ota_helper_script(config):
    """Write a helper shell script that wraps ota_image_tool.py for convenience.

    After building, users can run this script to convert the ESPHome .bin
    into a Matter .ota image suitable for HA's OTA provider.
    """
    script_path = CORE.relative_build_path("create_ota_image.sh")
    vendor_id = f"0x{config[CONF_VENDOR_ID]:04X}"
    product_id = f"0x{config[CONF_PRODUCT_ID]:04X}"
    sw_ver = config[CONF_SOFTWARE_VERSION]
    device_name = CORE.name

    script_content = f"""#!/usr/bin/env bash
# Auto-generated by ESPHome Matter component.
# Converts an ESPHome firmware binary into a Matter OTA image.
#
# Usage: ./create_ota_image.sh [input.bin] [output.ota]
#   input.bin  — defaults to .pioenvs/{device_name}/firmware.bin
#   output.ota — defaults to {device_name}.ota

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${{BASH_SOURCE[0]}}")" && pwd)"

# Find ota_image_tool.py in the esp_matter managed component
OTA_TOOL="$(find "$SCRIPT_DIR/managed_components" -path "*/connectedhomeip/src/app/ota_image_tool.py" -print -quit 2>/dev/null)"
if [ -z "$OTA_TOOL" ]; then
    echo "Error: ota_image_tool.py not found. Run a build first so managed_components are downloaded."
    exit 1
fi

INPUT="${{1:-$SCRIPT_DIR/.pioenvs/{device_name}/firmware.bin}}"
OUTPUT="${{2:-$SCRIPT_DIR/{device_name}.ota}}"

if [ ! -f "$INPUT" ]; then
    echo "Error: Input file not found: $INPUT"
    echo "Build the firmware first with: esphome compile your_config.yaml"
    exit 1
fi

echo "Creating Matter OTA image..."
echo "  Input:   $INPUT"
echo "  Output:  $OUTPUT"
...
echo "Done! OTA image written to: $OUTPUT"
echo ""
echo "To update via Home Assistant:"
echo "  1. Copy $OUTPUT to your HA machine"
echo "  2. Place it in the Matter Server's OTA updates directory"
echo "  3. The device will automatically check for and apply the update"
"""
    write_file_if_changed(script_path, script_content)
...
async def to_code(config):
    ...
    cg.add(var.set_software_version(config[CONF_SOFTWARE_VERSION]))
    ...
    # Write a helper script for creating Matter OTA images
    _write_ota_helper_script(config)
